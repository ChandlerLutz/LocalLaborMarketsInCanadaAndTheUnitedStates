...
felm.rhs <- "~ 0 + start_end:country + CA_2 + CA_3 + CA_4 + CA_5 + US_3 + US_4 + US_5 + US_6 + US_7 + US_8 + US_9 | 0 | (diff_log_census_emp_us | diff_log_census_emp_ca ~ census_bartik_ca + census_bartik_us) | state_abb"
...
star.out.2sls <- stargazer(bartik.2sls, type = "latex",
                           keep.stat = "n",
                           title = "Using only Census Bartiks: 2SLS Estimates -- Local Labor Market Effects of Sectoral Shifts Predicted at the National Level (Bartik):  1990 to 2011",
...
star_tex_write(star.out.2sls, file = "TexFiles/24-b-cenus_bartik_2sls.tex",
               headers = headers.tex)
