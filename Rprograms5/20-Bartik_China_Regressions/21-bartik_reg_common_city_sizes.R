...
##write to rds file
saveRDS(all.data[,unique(city_id)], "RdsFiles/21-common_pop_support_city_ids.rds")
...
f_stage1 <- function(DT) {
  ...
  star.out <- stargazer(mod1, mod2, mod3,
                        title = "First Stage Common City Size Estimates -- Changes in Local Employment and Sectoral Shifts Predicted at the National Level (Bartik): 1990 to 2011",
...
star.out.2sls <- stargazer(bartik.2sls, type = "latex",
                           keep.stat = "n",
                           title = "2SLS Common City Size Estimates of Changes in Local Labor Market Outcomes due to Changes in Employment (Two Bartik Instrument Estimates): 1990 to 2011",
