...
##    $Revisions:      1.0.0     $Date:  2017-04-26
...
us.ca.pval.ipw <- linearHypothesis(
    ...
    test = "F", vcov. = first.us.ca$clustervcv)
us.ca.pval.ipw <- us.ca.pval.ipw[4][2, 1] %>%
    sprintf("%0.3f", .) %>%
    paste0("$\\Delta$IPW from China to Other: US = Canada pval & \\multicolumn{2}{c}{", .,
           "} \\\\")
...
star.first <- capture.output(stargazer(first.us, first.ca, type = "latex",
                        label = "tab:china_first",
                        title = "First Stage ``China Syndrome'' Estimates: Change in Local Imports Predicted by Foreign Changes",
                        keep.stat = c("n", "f", "rsq"))) %>%
...
saveRDS(star.first, "RdsFiles/10-china_shock_stargazer_stage1.rds")
...
saveRDS(star.string, "RdsFiles/10-china_shock_stage1_star_insert_string.rds")
...
second.us <- lapply(lhs.vars$lhs, function(lhs) {
  ...
  stargazer(., type = "latex", keep.stat = "n",
            ...
            title = "Instrumental Variable Estimates of the Impact of Import Competition on Local Labor Markets in the U.S. and Canada: 1990 to 2007",
...
second.ca.us <- star_insert_row(second.ca.us,
    ...
    ##Add notes
    star_notes_tex(note.type = "threeparttable",
                   note = "See the notes for table \\ref{tab:gradients}. Sample consists of 264 metro areas in the United States and 82 in Canada observed  in  1990, 2000, and 2007. Not all dependent variables are available for all metro areas. See the notes to table \\ref{tab:china_first} for descriptions of the imports per worker variables. Controls include census division/region and decadal fixed effects. See the notes to table \\ref{tab:summ_stats} for data sources and definitions. Regressions are  weighted by start of period population 24-59. Heteroskedasticity robust standard errors clustered at the state/province level and are in parentheses. *, **, *** indicates statistical significance at the 10, 5, and 1 percent levels, respectively.") %>%
    star_sidewaystable

star_tex_write(second.ca.us, file = "TexFiles/10-china_shock_2sls.tex",
          headers = headers.tex)
